...
1. **确定 scope** — 检查最近提交（上次 tag 到 HEAD）涉及的文件路径。如有多个 scope 改动，对各个 scope 分别判断。无匹配时使用 `(root)`（即无前缀）。
2. **确定项目类型** — `detect` 自动检测仓库类型（code / docs），后续规则因类型而异。
3. **是否发版** — 参考[版本号规则](#版本号规则)判断。有逻辑改动则发版，仅 chore/typo/CI 则不发送。
4. **确定增量与预发布** — 参考[版本号规则](#版本号规则)推断 minor/patch、直发正式还是走预发布。
